  ...
  static async addTask({
    ...
    const result = await fetch(
      `http://localhost:4321/tasklists/${tasklistId}/tasks`,
      {
        ...
        body: JSON.stringify({ taskName, nomer })
      }
    );
  ...
  static async editTask({
    ...
    const result = await fetch(
      `http://localhost:4321/tasklists/${tasklistId}/tasks/${taskId}`,
      {
        ...
        body: JSON.stringify({ newTaskName, newNomer })
      }
    );
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    ...
    const nomer = prompt('Введите время от 8.00 до 20.00:');
    ...
    try {
      ...
      this.addTask(newTaskName, nomer);
    } catch (error) {
      console.error('ERROR', error);
    }
  };

   addTask = (taskName, nomer) => {
    document.querySelector(`#${this.tlID} ul`)
      .appendChild(
        this.renderTask({
          taskID: `${this.tlID}-T${this.tasks.length}`,
          taskName: [taskName, nomer]
        })
      );
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новую фамилию пациента:', oldTaskName[0]);
    const newNomer = prompt('Введите новое время от 8.00 до 20.00:', oldTaskName[1]);
    ...
    try {
      ...
      document.querySelector(`#${taskID} span`)
        .innerHTML = newTaskName + ',' + newNomer;
    } catch (error) {
      console.error('ERROR', error);
    }
  };
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Пациент '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить пациента';
